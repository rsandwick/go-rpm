...
func (enc *Encoder) Encode(v interface{}) error {
	return enc.EncodeNS(v, nil)
}

// EncodeNS writes the XML encoding of v to the stream.
//
// See the documentation for Marshal for details about the conversion
// of Go values to XML.
//
// Encode calls Flush before returning.
func (enc *Encoder) EncodeNS(v interface{}, ns map[string]string) error {
	if ns == nil {
...
func (p *printer) writeStart(start *StartElement) error {
	...
	if err := p.writeStartInner(start); err != nil {
		return err
	}
	p.WriteByte('>')
	return nil
...
// writeSelfClosingStart writes the given start element with its own closure.
func (p *printer) writeSelfClosingStart(start *StartElement) error {
	if start.Name.Local == "" {
		return fmt.Errorf("xml: start tag with no name")
	}
	if err := p.writeStartInner(start); err != nil {
...
func (p *printer) writeStartInner(start *StartElement) error {
	p.writeIndent(1)
	p.WriteByte('<')
